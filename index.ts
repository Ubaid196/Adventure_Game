...
let HealthPotionDropChance: number = 50; //Precentage
...
async function GAME() {
  while (running) {
    ...
    while (enemyHealth > 0) {
      ...
      if (userInput === "Attack!") {
        ...
        if (numHealthPotions > 0) {
          ...
          numHealthPotions--;
          console.log(
            "\t> You drink health potion, healing yourself for " +
    ...
    let inputNew: answer_1 = await inquirer.prompt([
      {
        name: "user_Input",
        type: "list",
        message: "What would you like to do?",
